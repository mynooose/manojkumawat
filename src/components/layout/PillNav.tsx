...
/**
 * Floating pill navigation. Server component — nothing here needs state.
 *
 * The header is pointer-events:none so the padding gutter does not swallow
  ...
  return (
    <header className="pointer-events-none fixed inset-x-0 top-0 z-[110] flex justify-center px-[clamp(14px,3vw,28px)] py-[14px]">
      ...
      >
        ...
        {/* Spec: links are hidden below 760px, logo and CTA remain. */}
        <div className="hidden flex-wrap items-center gap-[2px] font-mono text-[11.5px] min-[760px]:flex">
          {NAV_LINKS.map((link) => (
            <a
              key={link.href}
              href={link.href}
              className="rounded-pill px-[13px] py-2 text-text-2 transition-colors duration-[180ms] hover:bg-line-3 hover:text-text"
            >
              {link.label}
            </a>
          ))}
        ...
        <a
          href="#contact"
          className="flex-none rounded-pill bg-text px-5 py-[11px] text-[13.5px] font-medium whitespace-nowrap text-bg transition duration-200 hover:-translate-y-px hover:bg-accent hover:text-bg"
        >
          Let&apos;s talk
        </a>
      </nav>
